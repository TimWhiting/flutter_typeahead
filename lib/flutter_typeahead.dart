...
class LocationChangedNotifier extends SingleChildRenderObjectWidget {
  /// Creates a [SizeChangedLayoutNotifier] that dispatches layout changed
  /// notifications when [child] changes layout size.
  final void Function(Offset location) onChanged;
  const LocationChangedNotifier({Key key, Widget child, this.onChanged}) : super(key: key, child: child);

  @override
  _LocationChangedNotifierWithCallback createRenderObject(BuildContext context) {
    return _LocationChangedNotifierWithCallback(onLocationChangedCallback: onChanged);
  }
}

class _LocationChangedNotifierWithCallback extends RenderProxyBox {
  _LocationChangedNotifierWithCallback({
    RenderBox child,
    @required this.onLocationChangedCallback,
  })  : assert(onLocationChangedCallback != null),
        super(child);
  ...
  final void Function(Offset position) onLocationChangedCallback;
  ...
  void performLayout() {
    ...
    try {
      newOffset = hasSize ? localToGlobal(Offset.zero) : null;
    } on Exception catch (e) {
      print(e);
    }
    if (_oldOffset != null && newOffset != _oldOffset) onLocationChangedCallback(newOffset);
...
class _TypeAheadFieldState<T> extends State<TypeAheadField<T>> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    return LocationChangedNotifier(
      onChanged: (location) {
        print("Typeahead location changed");
        this._suggestionsBox.resize();
      },
